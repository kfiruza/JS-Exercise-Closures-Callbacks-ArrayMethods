...
function processSum(numberList, callback) {
  /* CODE HERE */
  return callback(numberList.reduce((total, num) => { return total +=num;}, 0));
}
...
function firstNamesAllCaps(runners) {
  /* CODE HERE */
  var allRunner = runners.map(function(person){
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  const don = runners.reduce((total, num) => {
  return total += num.donation }, 0);
  return don;
 ...
 * etc
*/
function counterMakerWithLimit(limitMax) {
  /* CODE HERE */
 var count = 0;
 return function counter(){
 if (count <= limitMax) {
 return count++;
  } else if (count = limitMax) {
  count = 0;
  return count++;
    }
  }
